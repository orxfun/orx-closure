...
fn owning_higher_order_function() {
    fn make_owning_function(data: Vec<i32>) -> ClosureResRef<Vec<i32>, usize, i32, &'static str> {
        Capture(data).fun_result_ref(|data: &Vec<i32>, i| data.get(i).ok_or("it was None!!!"))
    }
...
fn assert_higher_order_function<'a, F: Fn(usize) -> Result<&'a i32, &'static str>>(fun: F) {
